...
<template>
  <div class="relative">
    ...
    <ul
      ...
      role="listbox"
      tabindex="-1"
      class="autocomplete-dropdown menu bg-base-100 border border-base-300 rounded-box shadow-lg absolute w-full z-50 p-0"
    >
      <li
        ...
        role="option"
        :class="{
          'opacity-40 pointer-events-none': disabledGameIds.has(game.id) || franchiseLimitedGameIds?.has(game.id),
          'bg-primary text-primary-content': index === highlightedIndex && !disabledGameIds.has(game.id) && !franchiseLimitedGameIds?.has(game.id),
        }"
      ...
      >
        <span>
          {{ game.name }}
          <span v-if="disabledGameIds.has(game.id)" class="badge badge-neutral badge-sm ml-2">
            Already played
          </span>
          <span v-else-if="franchiseLimitedGameIds?.has(game.id)" class="badge badge-neutral badge-sm ml-2">
